...
function QrCreateDialog({ invoices, error, onCreate, onClose }: { invoices: Invoice[]; error: string; onCreate: (invoice: Invoice, amount: number) => Promise<void>; onClose: () => void }) {
  ...
  return <PartnerDebtDialog title="Tạo QR thanh toán" onClose={onClose}>
    {!invoice ? <p className="text-sm text-er">Không có hóa đơn còn nợ.</p> : <div className="space-y-4">
      ...
      {error && <p className="text-sm text-er">{error}</p>}<button type="button" disabled={amount <= 0 || amount > invoice.remaining || pending} onClick={submit} className="w-full rounded-lg bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50 min-h-11 lg:min-h-0">{pending ? "Đang tạo QR..." : "Tạo QR"}</button>
    </div>}
  </PartnerDebtDialog>;
}
...
function EntryDialog({ customerId, currentDebt, mode, error, onError, onClose }: { customerId: string; currentDebt: number; mode: "adjust" | "discount"; error: string; onError: (value: string) => void; onClose: () => void }) {
  const [amount, setAmount] = useState<number | null>(null); const [reason, setReason] = useState(""); const [pending, startTransition] = useTransition();
  const [clientRequestId] = useState(() => requestId("web-receivable-entry"));
  const signedAmount = mode === "discount" ? -Math.abs(amount ?? 0) : amount ?? 0;
  const type = mode === "discount" ? "settlement_discount" : signedAmount > 0 ? "adjustment_debit" : "adjustment_credit";
  function submit() { if (amount == null || (mode === "adjust" ? amount === currentDebt : signedAmount === 0) || !reason.trim() || pending) return; startTransition(async () => { const result = await createCustomerReceivableEntry({ customerId, ...(mode === "adjust" ? { targetDebt: amount } : { amount: signedAmount }), type, reason, clientRequestId }); if (!result.ok) return onError("Không thể tạo chứng từ. Thao tác này yêu cầu quyền quản lý."); onClose(); }); }
  return <PartnerDebtDialog title={mode === "discount" ? "Chiết khấu thanh toán" : "Điều chỉnh công nợ"} onClose={onClose}><div className="space-y-4"><DebtBalanceSummary value={currentDebt} /><label className="block text-sm font-medium">{mode === "adjust" ? "Công nợ mới" : "Số tiền"}<MoneyInput value={amount} min={mode === "adjust" ? -Number.MAX_SAFE_INTEGER : 0} onChange={setAmount} className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-right" /></label>{mode === "adjust" && <p className="text-xs text-slate-500">Nhập số âm nếu khách hàng có số dư/trả trước.</p>}<label className="block text-sm font-medium">Lý do<textarea value={reason} maxLength={200} onChange={(event) => setReason(event.target.value)} className="mt-1 min-h-24 w-full rounded-lg border border-border p-2" /><span className="mt-1 block text-right text-xs text-slate-400">{reason.length}/200</span></label>{error && <p className="text-sm text-er">{error}</p>}<DebtDialogFooter onCancel={onClose} onConfirm={submit} confirmLabel={pending ? "Đang lưu..." : "Xác nhận"} disabled={amount == null || (mode === "adjust" ? amount === currentDebt : amount === 0) || !reason.trim() || pending} /></div></PartnerDebtDialog>;
}
