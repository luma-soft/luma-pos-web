...
export function InstantProductSearch({ value, placeholder }: { value: string; placeholder: string }) {
  ...
  return (
    <div className="relative w-full min-w-0 flex-1 sm:min-w-[240px] sm:max-w-sm">
      ...
      <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={placeholder} className="min-h-11 w-full rounded-lg border border-border bg-surface py-2 pl-9 pr-3 text-sm outline-none transition focus:border-primary-500 focus:ring-2 focus:ring-primary-100 sm:min-h-0" autoComplete="off" />
    </div>
  );
}
