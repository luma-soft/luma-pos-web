...
import { createServiceSignatureCore } from "@/lib/services/field-operations";
import { serviceSignatureSchema } from "@/lib/services/schemas";
...
export async function POST(
  ...
  const parsed = serviceSignatureSchema.safeParse({
  ...
  return mobileFieldOperation(() => db.transaction((tx) =>
    createServiceSignatureCore(tx, { userId: gate.userId, role: gate.role }, parsed.data)
  ));
}
