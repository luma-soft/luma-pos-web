...
export function createNotificationOutboxCore(options: NotificationOutboxCoreOptions) {
  ...
  async function processNotificationMessage(
    ...
    const configuredRoles =
      notifications.roleRouting[category] as readonly Role[];
    ...
    const results = await Promise.all(deviceClaims.map(async ({ device, claim: deviceClaim }) => {
      ...
      const sendFence = await database.transaction(async (tx: DatabaseLike) => {
        ...
        const liveConfiguredRoles =
          liveNotifications.roleRouting[category] as readonly Role[];
        const liveAllowedRoles = liveConfiguredRoles
          .filter((role) => targetRoles.includes(role));
        if (!live) {
          await tx
            .update(mobilePushDeliveries)
