...
export function CameraPriceListClient({ models: initialModels, memoryLabels, canEdit }: { models: Model[]; memoryLabels: string[]; canEdit: boolean }) {
  ...
  function applyPrice() {
    ...
    setModels((current) => current.map((model) => model.id !== editing.model.id ? model : {
      ...model,
      variants: model.variants.map((variant) => variant.id === editing.variant.id ? { ...variant, price: value } : variant),
    }));
    setEditing(null);
