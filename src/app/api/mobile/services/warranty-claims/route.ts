import { createWarrantyClaim } from "@/lib/actions/services";
...
export async function GET() {
  ...
  return mobileOk({ rows: (await getServiceDashboard()).claims });
}
...
export async function POST(request: Request) {
  ...
  return mobileAction(await createWarrantyClaim(body as Parameters<typeof createWarrantyClaim>[0]));
}
