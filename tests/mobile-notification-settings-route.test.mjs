...
await check("non-manager staff cannot read or mutate notification administration", async () => {
  for (const blockedRole of ["cashier", "warehouse", "technician"]) {
    role = blockedRole;
    approvalCalls = 0;
    updateCalls = [];
...
await check("truthy primitives, arrays, unknown keys, and malformed nested values fail before approval", async () => {
  ...
  const malformedPayloads = [
    ...
    { roleRouting: { invoiceCreated: ["technician"] } },
    { roleRouting: { purchaseReceived: ["cashier"] } },
    { roleRouting: { purchaseReceived: ["technician"] } },
    { roleRouting: { debtChanged: ["technician"] } },
    { roleRouting: { qrPaymentConfirmed: ["warehouse"] } },
    { roleRouting: { qrPaymentConfirmed: ["technician"] } },
    { roleRouting: { qrPaymentException: ["cashier"] } },
    { roleRouting: { qrPaymentException: ["technician"] } },
