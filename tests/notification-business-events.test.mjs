...
const sale = await createOrderForUser(actor.id, {
  mode: "sale",
...
const [sourceItem] = sourceSale.ok
  ...
  : [];
const exchange = sourceSale.ok && sourceItem
  ? await createExchangeForUser(actor.id, {
...
ok(
  ...
  repeatEditOne.ok
    ...
    && repeatedEditEvents.every((event) => (
      /^debt-changed:supplier:[0-9a-f-]+:purchase_edit:[0-9a-f-]+:\d{4}-\d{2}-\d{2}T/.test(event.eventKey)
    )),
  JSON.stringify(repeatedEditEvents.map((event) => event.eventKey)),
);
...
const [returnableItem] = returnableSale.ok
  ...
  : [];
const customerReturn = returnableSale.ok && returnableItem
  ? await createReturnForUser(actor.id, {
