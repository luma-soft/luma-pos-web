...
export function ShiftPanel({ open, openingFloat, expected, openedAt }: {
  ...
  if (!open) {
    return (
      <div className="bg-surface border border-border rounded-card shadow-e1 p-5 max-w-md mb-5">
        ...
        <button disabled={pending} onClick={doOpen} className="mt-3 inline-flex h-11 items-center gap-2 rounded-lg bg-primary-600 px-4 text-sm font-semibold text-white disabled:opacity-50">
  ...
  return (
    <div className="bg-surface border border-border rounded-card shadow-e1 p-5 max-w-md mb-5">
      ...
      {err && <p className="text-xs text-er mt-2">{err}</p>}
      <button disabled={pending || counted === ""} onClick={doClose} className="mt-3 inline-flex h-11 items-center gap-2 rounded-lg bg-primary-600 px-4 text-sm font-semibold text-white disabled:opacity-50">
