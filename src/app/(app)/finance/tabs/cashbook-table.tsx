...
function Note({ row }: { row: CashbookRow }) {
  if (row.refType === "order" && row.refId) return <Link href={`${Routes.Sales}?tab=orders&orderId=${encodeURIComponent(row.refId)}&expandedOrder=${encodeURIComponent(row.refId)}`} className="text-primary-600 hover:underline">{row.note}</Link>;
