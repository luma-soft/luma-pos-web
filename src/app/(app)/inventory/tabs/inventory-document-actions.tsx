...
export function InventoryDocumentActions({ kind, id, code, capabilities, inFooter = false }: { inFooter?: boolean; kind: "internal-use" | "purchase-returns"; id: string; code: string; capabilities?: { canEdit: boolean; canDelete: boolean } }) {
  ...
  async function remove() {
    if (!capabilities?.canDelete || busy || !await confirm({ title: `Xóa phiếu ${code}?`, description: kind === "purchase-returns" ? "Phiếu sẽ bị xóa. Tồn kho, tiền hoàn và công nợ liên quan sẽ được hoàn lại." : "Phiếu sẽ bị xóa và số lượng đã xuất sẽ được hoàn lại vào kho.", confirmLabel: "Xóa phiếu", variant: "destructive" })) return;
    setBusy(true);
    setError("");
    try {
