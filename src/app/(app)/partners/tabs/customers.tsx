...
import { CustomerTypeBadge } from "../../customers/type-badge";
...
export async function CustomersTab({ searchParams }: { searchParams: SP }) {
  ...
  const href = (overrides: Record<string, string | undefined>) => {
    ...
    const merged: Record<string, string | undefined> = { tab: "customers", q: params.q, type: params.type, owing: params.owing, ...overrides };
    for (const [k, v] of Object.entries(merged)) if (v) sp.set(k, v);
    return `${Routes.Partners}?${sp.toString()}`;
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
        ...
        <Link href={Routes.CustomerNew} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary-600 hover:brightness-110 text-white text-sm font-medium transition active:scale-[0.98]"><UserPlus className="w-4 h-4" />{t("customers.createNew")}</Link>
      </div>

      <div className="flex gap-1 border-b border-border mb-4 overflow-x-auto">
        {TYPE_TABS.map((tab) => (
          <Link key={tab} href={href({ type: tab === "all" || tab === "owing" ? undefined : tab, owing: tab === "owing" ? "1" : undefined, page: undefined })}
            className={cn("px-4 py-2 text-sm font-medium border-b-2 -mb-px whitespace-nowrap", activeTab === tab ? "border-primary-600 text-primary-600" : "border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200", tab === "owing" && activeTab !== "owing" && "text-er")}>
      ...
      <form className="flex items-center gap-3 mb-4" action={Routes.Partners}>
        ...
        {activeTab !== "all" && activeTab !== "owing" && <input type="hidden" name="type" value={activeTab} />}
        {activeTab === "owing" && <input type="hidden" name="owing" value="1" />}
        <div className="relative w-full max-w-sm">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
          <input type="text" name="q" defaultValue={params.q ?? ""} placeholder={t("customers.searchPlaceholder")} className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-border bg-surface" />
        </div>
              ...
              return (
                <Link key={c.id} href={Routes.customer(c.id)} className="block bg-surface border border-border rounded-card p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0"><div className="font-medium truncate">{c.name}</div><div className="text-xs text-slate-400">{c.phone ?? c.code}</div></div>
          ...
          <div className="hidden lg:block bg-surface border border-border rounded-card overflow-x-auto">
            <table className="w-full min-w-170 text-sm">
              ...
              <tbody className="divide-y divide-border-soft">
                {rows.map((c) => {
                  const debt = Number(c.currentDebt); const limit = Number(c.debtLimit ?? 0);
                  return (
                    <tr key={c.id} className="hover:bg-surface-2">
                      <td className="px-4 py-3"><Link href={Routes.customer(c.id)} className="font-medium text-primary-600 hover:underline">{c.name}</Link><div className="text-xs text-slate-400">{c.code}</div></td>
                      ...
                      <td className={cn("px-4 py-3 text-right tabular-nums font-semibold", debt > 0 ? "text-er" : "text-slate-400")}>{debt > 0 ? formatCurrency(debt) : "—"}</td>
                      <td className="px-4 py-3 text-right tabular-nums text-slate-500">{limit > 0 ? formatCurrency(limit) : "—"}</td>
                    </tr>
                  );
                })}
              </tbody>
