...
beforeAll(async () => {
  ...
  for (const table of [schema.products, schema.productUnits, schema.productPrices,
    schema.priceBooks, schema.promotions, schema.productComboItems]) {
  ...
  await database.insert(schema.products).values([
    { id: productId, storeId, sku: "KNOWN", name: "Sản phẩm", baseUnit: "cái", costPrice: "90", lastPurchasePrice: "120", retailPrice: "200" },
...
test("manual prices and line discounts remain invoice-only changes for an authorized user", async () => {
  ...
  expect(result.rows[0]).toEqual({ cost_price: "90.00", last_purchase_price: "120.00", retail_price: "200.00" });
});
