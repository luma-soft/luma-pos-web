      ...
      <div className="px-6 py-10 sm:px-10 sm:py-12">
        ...
        <section className="mt-9 grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.3fr)]">
          <aside className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
            ...
            <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white p-3 text-sm"><Checkbox checked={includeUps} onChange={(event) => setIncludeUps(event.target.checked)} className="mt-0.5 h-4 w-4" /><span><span className="block font-bold text-[#14344d]">{t("ups.title")}</span><span className="mt-1 block leading-5 text-slate-500">{t("ups.description")}</span></span></label>
            <label className="mt-4 block text-sm font-bold text-slate-700">{t("cableMeters")}<NumberInput min={0} decimals={4} thousandSeparator={false} value={cableMeters} onChange={(value) => setCableMeters(value ?? 0)} className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 font-normal" /></label>
            <div className="mt-4 grid gap-2"><label className="flex cursor-pointer gap-2 text-sm text-slate-700"><Checkbox checked={includeRack} onChange={(event) => setIncludeRack(event.target.checked)}  />{t("accessories.rack")}</label><label className="flex cursor-pointer gap-2 text-sm text-slate-700"><Checkbox checked={includeMonitor} onChange={(event) => setIncludeMonitor(event.target.checked)}  />{t("accessories.monitor")}</label><label className="flex cursor-pointer gap-2 text-sm text-slate-700"><Checkbox checked={includeSurge} onChange={(event) => setIncludeSurge(event.target.checked)}  />{t("accessories.surge")}</label></div>
          </aside>
