...
export function RecentMovements({ movements }: { movements: MovementItem[] }) {
  ...
  useEffect(() => { setPage(1); }, [query, type, warehouse]);
  useEffect(() => {
        ...
        <div className="fixed inset-0 z-[80] bg-slate-950/30" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setDrawerOpen(false); }}>
          <aside role="dialog" aria-modal="true" aria-label={t("inventory.movementsTitle")} className="ml-auto flex h-full w-full max-w-[720px] flex-col bg-surface shadow-2xl">
            ...
            <div className="grid gap-2 border-b border-border bg-canvas/60 p-4 sm:grid-cols-[minmax(0,1fr)_160px_160px]">
              <label className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("inventory.actions.searchMovements")} className="h-10 w-full rounded-lg border border-border bg-surface pl-9 pr-3 text-sm outline-none focus:border-primary-500" /></label>
              <label className="relative"><Filter className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><select value={type} onChange={(event) => setType(event.target.value)} className="h-10 w-full appearance-none rounded-lg border border-border bg-surface pl-9 pr-8 text-sm outline-none"><option value="all">{t("inventory.actions.allTypes")}</option>{types.map((value) => <option key={value} value={value}>{t(`inventory.moveTypes.${value}` as never)}</option>)}</select></label>
              <label className="relative"><SlidersHorizontal className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><select value={warehouse} onChange={(event) => setWarehouse(event.target.value)} className="h-10 w-full appearance-none rounded-lg border border-border bg-surface pl-9 pr-8 text-sm outline-none"><option value="all">{t("inventory.actions.allWarehouses")}</option>{warehouses.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
            </div>
