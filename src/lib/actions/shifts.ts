...
export async function closeShiftForUser(userId: string, countedCash: number, note?: string): Promise<ActionResult<{ variance: number }>> {
  ...
  try {
    ...
    const profileId = await getProfileId(userId);
    if (!profileId) return { ok: false, error: "errors.invalidData" };
    const shift = await getCurrentShift(context.storeId, profileId);
...
export async function handoverShiftForUser(input: {
  ...
  try {
    const context = await resolveStoreContextForUser(input.userId);
    if (!context) return { ok: false, error: "errors.unauthorized" };
    ...
    const [shift, target, targetShift] = await Promise.all([
      getCurrentShift(context.storeId, profileId),
      db.select().from(profiles).where(and(eq(profiles.storeId, context.storeId), eq(profiles.id, input.targetProfileId))).limit(1),
      getCurrentShift(context.storeId, input.targetProfileId),
    ]);
    if (!shift) return { ok: false, error: "shifts.errors.noOpen" };
