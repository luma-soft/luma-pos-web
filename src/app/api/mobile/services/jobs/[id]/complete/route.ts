...
import { completeFieldServiceJobCore } from "@/lib/services/field-operations";
import { serviceCompletionSchema } from "@/lib/services/schemas";
...
export async function POST(
  ...
  const parsed = serviceCompletionSchema.safeParse({
  ...
  return mobileFieldOperation(() => db.transaction((tx) =>
    completeFieldServiceJobCore(tx, { userId: gate.userId, role: gate.role }, parsed.data)
  ));
}
